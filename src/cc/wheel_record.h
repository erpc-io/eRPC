...
struct wheel_record_t {
  ...
  size_t req_num;     ///< The request number of the wheel entry's sslot
  size_t abs_tx_tsc;  ///< For inserts, the requested TX timestamp

  /// Record an wheel insertion entry
  wheel_record_t(size_t req_num, size_t abs_tx_tsc)
      : record_tsc(rdtsc()),
        insert(true),
        req_num(req_num),
  ...
  wheel_record_t(size_t req_num)
      : record_tsc(rdtsc()), insert(false), req_num(req_num) {}
  ...
  std::string to_string(size_t console_ref_tsc, double freq_ghz) {
    ...
    if (insert) {
      ret << "[Insert for req" << req_num << ", at " << record_us
          << " us, abs TX " << abs_tx_us << " us]";
    } else {
      ret << "[Reap for req " << req_num << ", at " << record_us << " us]";
    }
