...
void Rpc<TTr>::pkt_loss_scan_reqs_st() {
  ...
  for (Session *session : session_vec) {
    ...
    for (SSlot &sslot : session->sslot_arr) {
      ...
      if (ms_since_enqueue < kPktLossTimeoutMs) continue;

      // If we're here, we suspect a packet loss for this request
      char issue_msg[kMaxIssueMsgLen];  // The basic issue message
      sprintf(
          issue_msg,
          "eRPC Rpc %u: Packet loss suspected for session %u, req %zu. Action",
          rpc_id, session->local_session_num, sslot.tx_msgbuf->get_req_num());
    }
  }
}
