...
void Rpc<TTr>::process_small_req_st(SSlot *sslot, pkthdr_t *pkthdr) {
  ...
  if (unlikely(pkthdr->req_num <= sslot->cur_req_num)) {
    ...
    if (pkthdr->req_num < sslot->cur_req_num) {
      ...
      LOG_REORDER(trace_file, "%s: Dropping.\n", issue_msg);
      return;
    } else {
      ...
      if (sslot->tx_msgbuf != nullptr) {
        ...
        LOG_REORDER(trace_file, "%s: Re-sending response.\n", issue_msg);
        ...
        LOG_REORDER(trace_file, "%s: Response not available yet. Dropping.\n",
                    issue_msg);
        return;
      }
    }
...
void Rpc<TTr>::process_large_req_one_st(SSlot *sslot, const pkthdr_t *pkthdr) {
  ...
  if (unlikely(!in_order)) {
    ...
    if (pkthdr->pkt_num != data_size_to_num_pkts(pkthdr->msg_size) - 1) {
      LOG_REORDER(trace_file, "%s: Re-sending credit return.\n", issue_msg);
    ...
    if (sslot->tx_msgbuf != nullptr) {
      ...
      LOG_REORDER(trace_file,
                  "%s: Dropping because response not available yet.\n",
                  issue_msg);
    }
    return;
  }
