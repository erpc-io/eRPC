...
static const size_t kPktHdrMagicBits =
    128 - (8 + kMsgSizeBits + 16 + 3 + 1 + 1 + kPktNumBits + kReqNumBits);
static const size_t kPktHdrMagic = 11;  ///< Magic number for packet headers

static_assert(kPktHdrMagicBits == 18, "");  // Just to keep track
...
enum PktType : uint64_t {
  ...
  kPktTypeFlushReq,    ///< Flush request
  kPktTypeFlushResp    ///< Flush response
};
...
static std::string pkt_type_str(uint64_t pkt_type) {
  switch (pkt_type) {
    ...
    case kPktTypeFlushReq:
      return std::string("flush request");
    case kPktTypeFlushResp:
      return std::string("flush response");
  }
...
struct pkthdr_t {
  ...
  uint64_t pkt_type : 3;             ///< The packet type
