...
void send_req(AppContext *c, size_t msgbuf_idx) {
  ...
  if (kAppVerbose) {
    printf("congestion: Thread %zu sending request using msgbuf_idx %zu.\n",
...
void thread_func(size_t thread_id, app_stats_t *app_stats, erpc::Nexus *nexus) {
  ...
  if (c.session_num_vec.size() > 0) {
    printf("congestion: Thread %zu: All sessions connected.\n", thread_id);
  } else {
    printf("congestion: Thread %zu: No sessions created.\n", thread_id);
  }
  ...
  for (size_t i = 0; i < FLAGS_test_ms; i += kAppEvLoopMs) {
    ...
    printf(
        "congestion: Thread %zu: Tput {RX %.2f, TX %.2f} Gbps. "
