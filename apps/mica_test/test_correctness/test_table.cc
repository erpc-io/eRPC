...
int main() {
  ...
  test_key_t *key_arr = (test_key_t *)alloc.hrd_malloc_socket(
      keyarr_shm_key, num_keys * sizeof(test_key_t), 0);
  test_val_t *val_arr = (test_val_t *)alloc.hrd_malloc_socket(
      valarr_shm_key, num_keys * sizeof(test_val_t), 0);
  ...
  for (int iter = 0; iter < num_iters; iter++) {
    ...
    for (int i = 0; i < num_keys; i += batch_size) {
      ...
      for (int j = 0; j < batch_size; j++) {
        out_result =
            table.set(key_hash_arr[j], key_arr[i + j], (char *)&val_arr[i + j]);
    ...
    for (int i = 0; i < num_keys; i += batch_size) {
      ...
      for (int j = 0; j < batch_size; j++) {
        ...
        out_result =
            table.get(key_hash_arr[j], key_arr[i + j], (char *)&temp_val);
        if (out_result != MicaResult::kSuccess) {
