 ...
 * A server mode peer is created by the ENet library when we get a ENet connect
   ...
   context's maps.
 * A connect/disconnect SM handler in the RPC layer is guaranteed to see a
   non-resetted session. A reset work item is generated by the SM thead when an
   ENet peer gets disconnected, which must happen **after** all the SM packets
   on that peer have been received.
